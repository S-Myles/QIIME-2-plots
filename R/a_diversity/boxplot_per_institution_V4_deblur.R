...
ASV_table <- read_tsv("./data/V4_deblur/feature-table.biom.tsv", skip=1) %>%
...
plot_richness(physeq_data, x = "provider", color="provider", measure="Chao1") + 
  ...
  coord_flip()+ 
  ggtitle("V4 Amplicons processed with deblur (Chao1 index on read counts)")
# Sving plot in /doc/a_diversity
ggsave("./doc/a_diversity/V4_deblur_Chao1.png")
...
plot_richness(physeq_data, x = "provider", color="provider", measure="Shannon") + 
  ...
  theme(legend.position = "none") +
  coord_flip() + 
  ggtitle("V4 Amplicons processed with deblur (Shannon index on read counts)")
# Sving plot in /doc/a_diversity
ggsave("./doc/a_diversity/V4_deblur_Shannon.png")