...
ASV_table <- read_tsv("./data/V4V5_MH_DADA2/feature-table.biom.tsv", skip=1) %>%
...
plot_richness(physeq_data, x = "provider", color="provider", measure="Chao1") + 
  ...
  coord_flip()+ 
  ggtitle("V4V5 Amplicons processed with DADA2 (Chao1 index on read counts)")
# Sving plot in /doc/a_diversity
#ggsave("./doc/a_diversity/V4V5_DADA2_Chao1.png")
...
plot_richness(physeq_data, x = "provider", color="provider", measure="Shannon") + 
  ...
  theme(legend.position = "none") +
  coord_flip() + 
  ggtitle("V4V5 Amplicons processed with DADA2 (Shannon index on read counts)")
# Sving plot in /doc/a_diversity
#ggsave("./doc/a_diversity/V4V5_DADA2_Shannon.png")